...
public class Parser {
    ...
    private List circolari;

    public Parser() {
        circolari = new ArrayList();
    }
    ...
    public List parseDocument(String filename)
        ...
        if (nodelist != null && nodelist.getLength() > 0) {
            for (int i = 0; i < nodelist.getLength(); i++) {
                ...
                circolare=getCircolare(element);
                circolari.add(circolare);
                
            }
        }
        return circolari;
    }
private Circolari getCircolare(Element element){
    Circolari circo;
    String tipo = getTextValue(element, "h4");
    String data = getTextValue(element, "time");
    String titolo = getTextValue(element, "a");
    String link = element.getAttribute("href");
    circo = new Circolari(data,titolo,tipo,link);
    return circo;
}
